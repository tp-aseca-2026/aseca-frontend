...
export function TransactionModal({
    ...
    const selectedStock = stocks.find((stock) => stock.ticker === defaultTicker);

    const title = lockedTicker
        ? mode === "buy"
            ? `Comprar más ${defaultTicker}`
            : `Vender ${defaultTicker}`
        : mode === "buy"
            ? "Registrar compra"
            : "Registrar venta";
    ...
    function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const ticker = lockedTicker
            ? defaultTicker.trim().toUpperCase()
            : String(formData.get("ticker") || "").trim().toUpperCase();
        ...
        <div style={overlayStyle}>
            <form onSubmit={handleSubmit} style={modalStyle} data-cy="transaction-modal">
                ...
                <p style={descriptionStyle}>
                    {lockedTicker
                        ? "Ingresá la cantidad que querés operar para esta acción."
                ...
                {lockedTicker ? (
                    <div style={lockedTickerBox} data-cy="modal-locked-ticker">
                        {defaultTicker} — {selectedStock?.companyName ?? "Sin nombre"}
                    </div>
                ) : (
                    <select
                        name="ticker"
